...
module.exports = {
   ...
   login: async (req, res, next) => {
       try {
        ...
        if (req.body.password !== user.password) {
            return res.status(400).send(400).send({ status: false, data: { message: 'Invalid cerdentainls' } })
        }
   ...
   updateUserProfile: async (req, res) => {
       try {
           ...
           if (!exist) {
               return res.status(404).send({ status: false, data: { message: 'user deatills not found' } });
           }
   ...
   deleteUserProfile: async (req, res) => {
       try {
           const  exist = await users.findById(req.user.id);
           if (!exist) {
               return res.status(400).send({ status: false, data: { message: 'user details not found' } });
           }
   ...
   changePassword: async (req, res) => {
       try {
           ...
           if (user.password !== req.validateChangePassword.oldPassword) {
               return res.status(400).send({ status:  false, data: { message: 'invalid old passwod ' } })
           }
           const updateObj = {
               passwod: req.validateChangePassword.newPassword,
               updatedAt: new Date()
           }
